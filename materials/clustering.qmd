...
* _NOTE_: k-means belongs to a class of algorithms called `stochastic` or `randomized` algorithms, i.e. each time you run the algorithm, you will get a different answer. This is different to PCA where you will get the same answer

* Sometimes the problems are so hard that you need these randomized algorithms. There are no easy ways to solve these problems.

* This shows up a lot in machine learning (e.g. neural networks and large language models).

* The _cost_ function and the landscape is very rugged. You can get stuck in _local minima_.
