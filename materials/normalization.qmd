...
* _Brain teaser_: How to encode 3 different values for 3 categories such that they are _equidistant_ from each other? Just coding them as 1, 2 and 3 may denote that the last category is more important (higher value) than the first.

* Most ML models need numbers. Categorical values (e.g. `"red"`, `"green"`) are non-numeric and may have no natural order.

Create one binary feature per category so the model sees independent numeric inputs (no implied order) (_one-hot-encoding_).
