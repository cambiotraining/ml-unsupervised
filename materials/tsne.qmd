...
- Perplexity in t-SNE acts like a control for the effective number of nearest neighbours.

:::
<!--end callout -->


## Interactive figure showing tSNE and perplexity
...
for perp in perplexities:
    ...
    embeddings[perp] = tsne.fit_transform(X)

# 3) Build Plotly figure with frames, controlled by a slider
first_perp = perplexities[0]
first_xy = embeddings[first_perp]
...
fig = go.Figure(
    ...
    layout=go.Layout(
        title=f"t-SNE with Perplexity = {first_perp}",
        xaxis_title="t-SNE 1",
        yaxis_title="t-SNE 2",
        ...
        sliders=[dict(
            active=0,
            currentvalue={"prefix": "Perplexity: ", "xanchor": "right"},
            pad={"t": 30},
            steps=[
                dict(
                    label=str(perp),
                    method="animate",
                    args=[[f"perp-{perp}"], {"mode": "immediate", "frame": {"duration": 0, "redraw": True}}],
                )
                for perp in perplexities
            ],
        )],
    ...
    frames=[
        go.Frame(
            ...
            layout=go.Layout(title_text=f"t-SNE with Perplexity = {perp}"),
        )
        for perp in perplexities
    ],
)
...
## What does tSNE looks like compared to PCA?
